...
- Авто-переключение профиля по Wi-Fi SSID (подключился к сети → профиль активируется автоматически)
...
- Подключение к сохранённым и новым сетям; пароли хранятся зашифрованными через Windows DPAPI или системный keyring
...
| Netstat | Таблица активных соединений (Протокол / Локальный / Удалённый / Состояние) |
| ARP таблица | Кэш ARP с маппингом IP → MAC → Интерфейс |
| Сетевые адаптеры | Параметры всех сетевых интерфейсов (аналог `ipconfig /all`) |
| Таблица маршрутов | Таблица маршрутизации Windows (IPv4 / IPv6) |
...
| DNS Cache | Просмотр и очистка DNS-кэша Windows |
| IP Batch Check | Массовая проверка доступности IP из CSV или Excel: ping + reverse DNS, экспорт результатов |
...
- Сворачивание в трей, запуск свёрнутым, запуск с Windows
- Настройка интервала авто-сканирования Wi-Fi
...
keyring >= 25.0  # Резервное хранилище паролей (если pywin32 недоступен)
openpyxl >= 3.1  # Поддержка Excel в IP Batch Check
```
...
- Если `pywin32` недоступен — используется системный **keyring** (Windows Credential Manager)
...
- Устаревшие профили с base64-кодированием автоматически мигрируют при первом подключении
...
```bash
# Установить зависимости для сборки (один раз)
...
Или просто запустить `build.bat`. Результат: `dist\NetConneXion\NetConneXion.exe`.
