...
class TrayIcon:
    ...
    def _build_menu(self) -> "pystray.Menu":
        return pystray.Menu(
            ...
            Item(
                lambda _: (
                    f"Профиль: {self._current_profile}"
                    if self._current_profile
                    else "Нет активного профиля"
                ),
                None,
                enabled=False,
            ),
    ...
    def update_profile(self, profile_name: str) -> None:
        ...
        if self._icon is not None:
            self._icon.title = (
                f"{self._app_name} – {profile_name}" if profile_name else self._app_name
            )
    ...
    def update_icon_color(self, success: bool = True) -> None:
        ...
        if self._icon is None or not _HAS_PIL:
            return
        color = (76, 175, 80, 255) if success else (244, 67, 54, 255)
